"""kuli.or_batch — fan out many prompts to one OpenRouter model in parallel.

Model is shared across all prompts (-m or OPENROUTER_MODEL). A shared
--system/--context prefix is cached after the first call. Prompts: one per
line, or split on --delimiter.
"""
import argparse
import shutil
...
def cli_path():
    path = shutil.which("ask-or")
    if not path:
        die("ask-or not found on PATH", 1)
    return path


def parse_args():
    p = argparse.ArgumentParser(prog=PROG, description="Fan out prompts to an OpenRouter model.")
    p.add_argument("--model", "-m", help="OpenRouter model slug for all (or env OPENROUTER_MODEL)")
